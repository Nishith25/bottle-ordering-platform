...
import {
  ...
  skipNextCustomerSubscriptionDelivery,
...
export function SubscriptionProvider({
  ...
  const removeSubscriptionCoupon =
    useCallback(() => {
      setPendingSubscriptionDraft(
        (currentDraft) => {
          if (!currentDraft) {
  ...
  const pauseSubscription =
    useCallback(
      async (
        ...
        if (!token) {
          setError(
            "Please log in to pause your subscription."
          );
        ...
        try {
          ...
          setError(
            requestError instanceof Error
              ? requestError.message
              : "Unable to pause this subscription."
          );
  ...
  const resumeSubscription =
    useCallback(
      async (
        ...
        if (!token) {
          setError(
            "Please log in to resume your subscription."
          );
  ...
  const skipNextDelivery =
    useCallback(
      async (
        ...
        if (!token) {
          setError(
            "Please log in to skip a subscription delivery."
          );
        ...
        try {
          const result =
            await skipNextCustomerSubscriptionDelivery(
              token,
              subscriptionId
            );
          ...
          setError(
            requestError instanceof Error
              ? requestError.message
              : "Unable to skip the next delivery."
          );
