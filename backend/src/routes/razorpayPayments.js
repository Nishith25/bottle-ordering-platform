// backend/src/routes/razorpayPayments.js
...
function appendReturnParameters(
  returnUrl,
  parameters
) {
  const separator =
  ...
  try {
    await mongoSession.withTransaction(
      async () => {
        ...
        /*
         * Inventory ownership is now
         * transferred to the final Order.
         */
        paymentSession.inventoryReserved =
          false;
...
<body>
  ...
  <script>
    const checkoutOptions =
      ${safeJson(checkoutData)};

    const sessionToken =
      ${safeJson(req.params.sessionToken)};

    const returnUrl =
      ${safeJson(paymentSession.returnUrl)};
    ...
    checkoutOptions.modal = {
      ondismiss: async function () {
        ...
        try {
          ...
          // The expiry worker still releases stock.
        }
    ...
    razorpayCheckout.on(
      ...
      async function (response) {
        ...
        try {
          ...
          // Webhook or expiry worker will reconcile it.
        }
  ...
  async (req, res, next) => {
    try {
      ...
      return res.status(200).json({
        ...
        message:
          "Payment session marked as abandoned.",
...
async function cleanupExpiredPaymentSessions() {
  ...
  for (
    const paymentSession of expiredSessions
  ) {
    try {
      if (
        paymentSession.razorpayOrderId
