...
const subscriptionAddressSchema = new mongoose.Schema(
  {
...
const subscriptionSchema = new mongoose.Schema(
  {
    ...
    deliveriesPerCycle: { type: Number, required: true, min: 1 },
    items: {
      ...
      validate: {
        ...
        message: "A subscription must contain at least one bottle.",
      },
    },
    preferredDay: { type: String, required: true, trim: true },
    preferredSlot: { type: String, required: true, trim: true },
    deliveryAddress: {
    ...
    amountBeforeCoupon: { type: Number, default: 0, min: 0 },
    ...
    totalPerCycle: { type: Number, required: true, min: 0 },
    recurringTotalPerCycle: { type: Number, default: 0, min: 0 },
    paymentMethod: {
      type: String,
      enum: ["upi_autopay", "card_mandate"],
      required: true,
    },
    paymentStatus: {
      type: String,
...
subscriptionSchema.pre("validate", function ensureLegacyTotals(next) {
  if (!Number.isFinite(this.amountBeforeCoupon) || this.amountBeforeCoupon <= 0) {
    this.amountBeforeCoupon = Math.max(
      0,
      Number(this.originalTotal || 0) - Number(this.savings || 0)
    );
  }

  if (
    !Number.isFinite(this.recurringTotalPerCycle) ||
    this.recurringTotalPerCycle <= 0
  ) {
    this.recurringTotalPerCycle =
      this.amountBeforeCoupon || Number(this.totalPerCycle || 0);
  }
...
subscriptionSchema.index({ "coupon.code": 1, createdAt: -1 });
